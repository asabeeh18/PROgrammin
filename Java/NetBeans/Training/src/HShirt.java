...
//        }
     ...
     * @param end the end of the interval return true if overlap, else false.
     */
...
//        }
        ...
        {
            if (intersection(start, intervalNode.start, intervalNode.end))
            {
                return true;
            }
    ...
    /**
     * Returns if there is an intersection in the two intervals Two intervals
     * such that one of the points coincide: eg: [10, 20] and [20, 40] are NOT
     * considered as intersecting.
     */
    private boolean intersection(int start, int intervalStart, int intervalEnd)
    {
        return start <= intervalEnd && start >= intervalStart;
    }
    ...
    public static void main(String[] args) {
        ...
        for(int a0 = 0; a0 < q; a0++){
            ...
            {
                ist.add(in.nextInt(),in.nextInt());
            }
            int c=0;
            for(int i=0;i<n;i++)
            {
                if(ist.overlap(z[i],z[i]))    
                    c++;
            }

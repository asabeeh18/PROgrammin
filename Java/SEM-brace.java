//Again Jigar Maths Eval .
import javax.script.ScriptEngineManager;
import javax.script.ScriptEngine;
import java.util.*;
class SEM
{
	public static String replaceEXP(int left,int right,String eq)throws Exception//remove right and left too!!
	{
		return eq.substring(0,left+1)+solve(eq.substring(left+1,right-1))+eq.substring(right,eq.length());
	}	
														...
														//Using ^ and log				
	{
		...
		if(i==-1)
			return ;
		//brace solver for "pre" expression
		if(eq.charAt(i)==')')
		{
			int rbrace=i;
			while(eq.charAt(i)!='(')
				i--;
			eq=replaceEXP(i,rbrace,eq);
		}
		//pre finder
